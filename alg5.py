...
class Alg5MergeTest(object):
    def startAlg(self, image_files):
        print("Algorithm5 (wavelet2 decomposition) starting.")
        ...
        for family in ['haar']: #pywt.families():
            ...
            for waveletchoice1 in pywt.wavelist(family):
                ...
                for j in range(num_files):
                    ...
                    # if (newdecomp1[0].shape[0] % 2 == 1):
                    #     newdecomp1 = newdecomp1[0][0:-1,:], (newdecomp1[1][0][0:-1,:],newdecomp1[1][1][0:-1,:],newdecomp1[1][2][0:-1,:])
                    # if (newdecomp1[0].shape[1] % 2 == 1):
                    #     newdecomp1 = newdecomp1[0][0:-1,:], (newdecomp1[1][0][:,0:-1],newdecomp1[1][1][:,0:-1],newdecomp1[1][2][:,0:-1])
                    # print("newshape",newdecomp1[0].shape)
                    # print("currshape",recompimg.shape)
                    shaperun = pywt.idwt2(newdecomp1, waveletchoice, mode='per')
                    print("shapeshape",shaperun.shape)
                    # recompimg = np.zeros((shaperun.shape[0],shaperun.shape[1],3))
                    # print("recomp", type(recompimg), recompimg.shape)
